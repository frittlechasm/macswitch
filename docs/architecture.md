...
- Shortcut preferences: persists the selected shortcut in `UserDefaults`, exposes preset selection from the Settings window, and marks Command-Tab as experimental in both the picker and active status.
...
- Preset-based Switcher Shortcut configuration: keeps hotkey changes simple. Ordinary shortcuts use Carbon. Command-Tab and Command-Shift-Tab use a public active Core Graphics event tap that consumes matching events and supports forward and reverse selection. Settings labels Command-Tab as experimental and explains that event-tap behavior may vary by macOS version. Event-tap creation failure keeps the current shortcut and directs the user to an alternate preset or the Option-Tab reset.

...
- Diagnostics: writes privacy-safe counts, failures, and per-opening overlay latency to the system log without storing history or window content. Only the four numeric latency measurements are explicitly public and queryable in persisted logs; general diagnostic messages retain the system's private treatment.
